...
void CElementalSystem::LoadOptions()
{
	...
	m_iEtramuScroll_DropRate = GetPrivateProfileInt("GameServerInfo","EtramuScroll_DropRate",0,gDirPath.GetNewPath(FILE_SERVER_COMMON));
...
BOOL CElementalSystem::Drop(LPOBJ lpObj,LPOBJ lpTargetObj)
{
	...
	if( lpObj->MapNumber == MAP_INDEX_ACHERON )
	{
		...
		{
			MsgOutput(lpTargetObj->m_Index,"Dropping Random Penta -> ElementalSystem.cpp:453");
			int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
			BYTE SocketBonus = (lpObj->m_btAttribute==ELEMENT_NONE) ? (1+rand()%5) : lpObj->m_btAttribute;
			BYTE SocketOption[5];
			...
			ItemSerialCreateSend(lpObj->m_Index, lpObj->MapNumber, lpObj->X, lpObj->Y, ITEMGET(12,201+rand()%13), 0, 1,
				0, 0, 0, iMaxHitUser, 0, 0,SocketBonus,SocketOption);
			return true;
		}
	}
...
int CElementalSystem::GetAttackDamage(LPOBJ lpObj,LPOBJ lpTargetObj,int targetdefense,CMagicInf* lpMagic,BYTE btAttackElement,BYTE btDefenseElement)
{
	...
	//Elemental Damage Base modif
	//ElementalDamage += lpObj->Strength;
	...
	__try
	{
		...
		//MsgOutput(lpObj->m_Index,"[Debug][ElementalFormula]: EMax: %d AddMax: %d",lpObj->m_ElementalDamageMax,AddDamageMax );
		//MsgOutput(lpObj->m_Index,"[Debug][ElementalFormula]: EMin: %d AddMin: %d",lpObj->m_ElementalDamageMin,AddDamageMin);
		//MsgOutput(lpObj->m_Index,"[Debug][ElementalFormula]: Base Damage: %d",ElementalDamage);
	}
	__except ( sub=1, 1 )
	{
	...
	//MsgOutput(lpObj->m_Index,"[Debug][ElementalFormula]: Increase: %d%% TargetDef: %d",IncreaseDamage,targetdefense);
	ElementalDamage = ElementalDamage*IncreaseDamage/100; // IncDmg/100 default 
	ElementalDamage -= targetdefense;

	//Test Raw Elemental DMG LF ELDMG
	//ElementalDamage = lpObj->Strength + lpObj->Dexterity + lpObj->Vitality + lpObj->Energy;
	//ElementalDamage /= 10;
	//MsgOutput(lpObj->m_Index,"[Debug][ElementalFinal]: %d",ElementalDamage);
...
void CElementalSystem::ErrtelMix(LPOBJ lpObj)
{
	...
	lpObj->ChaosSuccessRate = 100; //Default: 50
	int nChaosNeedMoney = 100000;
	...
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		//DEBUG
		MsgOutput(lpObj->m_Index,"[ElementErtelMix]Creating ertel of type %d",iMithrilType);
		BYTE SocketBonus = iMithrilType | 0x10;
		BYTE SocketOption[5];
		MsgOutput(lpObj->m_Index,"[ElementErtelMix]Type %d | 0x10 = %d w/ opt: %d",iMithrilType,SocketBonus,GetErtelOption(type,1));
...
{
	MsgOutput(lpObj->m_Index,"[ElementRankUp]Requested rank %d",btRank);
	lpObj->ChaosLock = TRUE;
...
//	LogAddC(2, "[DEBUG] Rate2: %d", iPentagramLuckyCharmRate);
	...
	MsgOutput(lpObj->m_Index,"[ElementRankUp]SocketBonus_prev: %d",Ertel->m_SocketBonus);
	if ( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		if( iPentagramChaosCharm > 0 )
		{
		...
		BYTE SocketBonus = (16 * btRank) + (int)(Ertel->m_SocketBonus%16); //Default: Ertel->m_SocketBonus
		BYTE SocketOptions[MAX_SOCKET_COUNT];

		MsgOutput(lpObj->m_Index,"[ElementRankUp]SocketBonus_current: %d",SocketBonus);
		for(int i = 0; i < 5; i++)	{
			SocketOptions[i] = Ertel->m_SocketOption[i];
		}
		...
		MsgOutput(lpObj->m_Index,"[ElementRankUp]Saving with SOpt: %d SBonus: %d",SocketOptions[SocketBonus%5],SocketBonus);
