...
void CommandManager::Run(LPOBJ lpUser, LPSTR Text)
{
	...
	if( CheckStatus != 1)
	{
		...
		{
			MsgOutput(lpUser->m_Index, "Your level is too small to use this command");
		}
	...
	case Command::Reset:
		{
			if (lpUser->Level < 400 )
			{				
				MsgOutput(lpUser->m_Index, "You need level 400.");
				return;
			}
			lpUser->Level		= 1;
			lpUser->Experience	= 0;
			...
			lpUser->Y			= Y;
			lpUser->Dir			= Dir;
			MsgOutput(lpUser->m_Index, "Gratz! You now have %d resets. Switching character.",lpUser->iResetCount);
