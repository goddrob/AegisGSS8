...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTargetObj,int MSBDamage)
{
	...
	//CUSTOM PK OPTIONS AEGISPK

	
	PARTY_STRUCT* lpParty = &gParty.m_PartyS[lpTargetObj->PartyNumber];
	int pId = -1;

	for (int i=0; i<5; i++){
		pId = lpParty->Number[i];
		if (pId < 0) continue;
		LPOBJ lpPartyObj = &gObj[pId];
		if( lpPartyObj != NULL )
				{
					if( lpPartyObj->Connected >= PLAYER_PLAYING ) {
						MsgOutput(lpPartyObj->m_Index,"[PK]%s killed %s in party of %d",lpObj->Name,lpTargetObj->Name,lpParty->Count);
					}
		}
	}

	
	MsgOutput(lpObj->m_Index,"[PK]:%s killed %s",lpObj->Name,lpTargetObj->Name);
	//MsgOutput(lpTargetObj->m_Index,"[PK Info]:%s killed %s [PtCount:%d]",lpObj->Name,lpTargetObj->Name,lpParty->Count);
